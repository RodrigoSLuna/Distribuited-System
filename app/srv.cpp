...
void ResponseNrUsers(TSocket cliSock) {
  char responseNrUsers[BUFSIZE];
  snprintf(responseNrUsers, BUFSIZE, "%i \n", users_registered);
...
void ResponseAllUsers(TSocket cliSock) {
  int i;
  for (i=0; i < users_registered; i++) {
    char responseThisUser[BUFSIZE];
    snprintf(responseThisUser, BUFSIZE, "%s %s %s \n",
           users[i].name, users[i].ip, users[i].port);
    int res_len = strlen(responseThisUser);

    printf("%s",responseThisUser);
...
void RemoveUser(char *str) {
  ...
  for (i=0; i<users_logged; i++) {
    printf("\nTENTATIVA REMOCAO: %s|%s, %d\n", tok, users[i].name, strcmp(str, users[i].name));
    if (!strcmp(str, users[i].name)) {
      printf("oi\n");
      users[i].name[0] = '\0';
      users[i].ip[0] = '\0';
      users[i].port[0] = '\0';
